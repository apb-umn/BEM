...
τb_val,τw_val = 0.5444444444444444,0.3111111111111111
...
function push_if_unique_and_valid!(df::DataFrame, data::NamedTuple, X̂t::AbstractMatrix)
    ...
    #Condition 3: Values
    is_value_valid = abs((data.value./data.Vss-1))<1.

    # Push only if both hold
    if !is_duplicate && is_matrix_valid && is_value_valid
        push!(df, data)
    end
end

push_if_unique_and_valid!(results_df, data, FO.X̂t./ZO.X̄)

CSV.write(path*"grid_results_with_values.csv", results_df)
